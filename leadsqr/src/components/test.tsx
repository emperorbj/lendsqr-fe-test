...
const Table = ({ user }: TableProps) => {
    // State to keep track of which user's action menu is open
    const [open, setOpen] = useState(false);
    const [detailsOpen, setDetailsOpen] = useState<number | null>(null); // <-- Add state here
                    ...
                    {user.map((item) => (
                        <tr key={item.id}>
                            ...
                            <td className="actions">
                                <img
                                    src={threedots}
                                    onClick={() => setDetailsOpen(detailsOpen === item.id ? null : item.id)} // <-- Update onClick here
                                    alt="Actions"
                                />
                                {
                                    detailsOpen === item.id && ( // <-- Condition to show menu for specific user
